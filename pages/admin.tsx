...
export default function AdminPage({ user, bookings, isLoading }) {
  ...
  const bookingModal = React.useContext(BookingModalContext)
  ...
  const deleteBooking = () => {
    try {
      setLoading(true)
      client.mutate({ mutation: DELETE_BOOKING, variables: { bookingId: booking.id}})
      setLoading(false)
    } catch(error) {
      setLoading(false)
  ...
  const updateBooking = async (
    ...
    try {
      setLoading(true)
      await client.mutate({ mutation: UPDATE_BOOKING, variables: {id: booking.id, email: submitBooking.email,
